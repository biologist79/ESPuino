...
* 24.07.2026: Add a configurable default action for unknown/unassigned RFID tags ("Default for unknown cards" under the general options) - scanning a tag with no stored assignment now plays a preconfigured file or folder instead of only indicating an error. A file plays as a single track, a folder plays all its tracks sorted (auto-detected from the path). Empty by default (unchanged behaviour: an unknown tag errors out); only acts in normal mode, so an unknown tag still just leaves Bluetooth mode as before.
