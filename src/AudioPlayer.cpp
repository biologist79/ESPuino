    ...
    for (;;)
    {
        ...
        if (trackQStatus == pdPASS || gPlayProperties.trackFinished || trackCommand != 0)
        {
            ...
            if (gPlayProperties.trackFinished)
            {
                ...
                { // Don't save for AUDIOBOOK_LOOP because not necessary
                ...
                { // Go to sleep if "sleep after track" was requested
                    System_RequestSleep();
                    break;
                }
            ...
            case NEXTTRACK:
                ...
                if (gPlayProperties.repeatCurrentTrack)
                { // End loop if button was pressed
            ...
            if (gPlayProperties.currentTrackNumber >= gPlayProperties.numberOfTracks)
            { // Check if last element of playlist is already reached
                ...
                { // Check if sleep after current track/playlist was requested
            ...
            {
                // Files from SD
                if (!gFSystem.exists(*(gPlayProperties.playlist + gPlayProperties.currentTrackNumber)))
                { // Check first if file/folder exists
        ...
        {
            if (settleCount++ == 50)
            { // Hack to give audio some time to settle down after playlist was generated
                gPlayProperties.playlistFinished = true;
                gPlayProperties.playMode = NO_PLAYLIST;
                settleCount = 0;
            }
