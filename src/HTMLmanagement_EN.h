					...
					<form id=\"explorerUploadForm\" method=\"POST\" enctype=\"multipart/form-data\" action=\"/explorer\">\
                        <div class=\"input-group\">\
                            ...
                            <span class=\"input-group-btn\">\
                                <span class=\"btn btn-secondary\" onclick=\"let input = $(this).parent().find('input[type=file]')[0]; input.webkitdirectory=false; input.click();\" title=\"Upload one ore more files\">Files</span>\
                                <span class=\"btn btn-secondary\" onclick=\"let input = $(this).parent().find('input[type=file]')[0]; input.webkitdirectory=true; input.click();\" title=\"Upload directory with all files and subdirectories\">Directory</span>\
                                <span class=\"btn btn-primary\" onclick=\"$(this).parent().find('input[type=file]').submit();\" title=\"Start Upload\">Upload</span>\
                    ...
                    <div class=\"tab-content\" id=\"SubTabContent\">\
                        <div class=\"tab-pane show active\" id=\"rfidmusic\" role=\"tabpanel\">\
                            ...
                            <select class=\"form-control\" id=\"playMode\" name=\"playMode\">\
                                ...
                                <option class=\"option-folder\" value=\"12\">Single track of a directory (random)</option>\
...
\
        ...
        <div class=\"container\" id=\"ftpStart\">\
            <div class=\"form-group col-md-12\">\
                ...
                <div>\
                    Enables FTP-server until device is restarted.\
                </div>\
                <br>\
                <div class=\"text-center\">\
...
\
			...
			success: function(data, textStatus, jqXHR) {\
                ...
                var progressText = \"Upload done (\" + timeText + \", \" + Kbytes_per_second.toFixed(2) + \"  KB/s).\";\
...
\
		$('#explorerTree').jstree({\
				\"core\" : {\
						\"check_callback\" : true,\
						'force_text' : true,\
                        'strings' : { \"Loading ...\" : \"is being loaded...\" },\
						\"themes\" : { \"stripes\" : true },\
						'data' : {	text: '/',\
									state: {\
										opened: true\
