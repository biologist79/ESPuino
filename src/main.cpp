...
#define DREHENCODER_CLK                 32
#define DREHENCODER_DT                  34
#define DREHENCODER_BUTTON              33

// GPIOs (Control-buttons)
#define PAUSEPLAY_BUTTON                4
...
static const char backupFile[] PROGMEM = "/sd/backup.txt"; // File is written every time a (new) RFID-assignment via GUI is done
...
void deepSleepManager(void) {
    if (gotoSleep) {
        ...
        spiSD.end();*/
        SPI.end();
        pinMode(RFID_MOSI, INPUT_PULLDOWN);
        pinMode(RFID_MISO, INPUT_PULLDOWN);
        pinMode(RFID_SCK, INPUT_PULLDOWN);
...
void setup() {
    ...
    if (!SD.begin("/sd", true)) {
        Serial.println("SDMMC Card Mount Failed");
        return;
    } else {
        Serial.println("SDMMC Card Mount Success");
    }
