...
static void handleCoverImageRequest(AsyncWebServerRequest *request) {
  ...
  [coverFile](uint8_t *buffer, size_t maxLen, size_t total) -> size_t {
     ...
     if (!file.available()) {
        file.close();
        Log_Println("cover image serving finished, close file", LOGLEVEL_NOTICE);
     }
