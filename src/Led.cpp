...
void Led_Init(void) {
		...
		if (nvsILedBrightness) {
			...
			Log_Printf(LOGLEVEL_INFO, "%s: %d", (char *) FPSTR(initialBrightnessfromNvs), nvsILedBrightness);
		} else {
		...
		if (nvsNLedBrightness != 255) {
			...
			Log_Printf(LOGLEVEL_INFO, "%s: %d", (char *) FPSTR(restoredInitialBrightnessForNmFromNvs), nvsNLedBrightness);
		} else {
